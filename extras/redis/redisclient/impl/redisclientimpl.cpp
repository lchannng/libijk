...
namespace
{
    ...
    int socketReadSomeImpl(int socket, char *buffer, size_t size,
            size_t timeoutMsec)
    {
        //struct timeval tv = {static_cast<time_t>(timeoutMsec / 1000),
        //    static_cast<__suseconds_t>((timeoutMsec % 1000) * 1000)};
        struct timeval tv = {static_cast<time_t>(timeoutMsec / 1000),
                             (timeoutMsec % 1000) * 1000};
        int result = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv));
            ...
            asio::error_code &ec)
    {
        ...
        for(;;)
        {
            int result = socketReadSomeImpl(socket,
                    static_cast<char *>(buffer.data()) + bytesRecv,
    ...
    int socketWriteImpl(int socket, const char *buffer, size_t size,
            size_t timeoutMsec)
    {
        //struct timeval tv = {static_cast<time_t>(timeoutMsec / 1000),
        //    static_cast<__suseconds_t>((timeoutMsec % 1000) * 1000)};
        ...
        */
        ...
        FD_ZERO(&rfds);
        FD_SET(socket, &rfds);
        result = select(1, NULL, &rfds, NULL, &tv);
        if (result > 0)
        {
            ...
            asio::error_code &ec)
    {
        ...
        {
            int result = socketWriteImpl(socket,
                    static_cast<const char *>(buffer.data()) + bytesSend,
