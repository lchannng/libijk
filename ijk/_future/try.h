...
public:
    ...
    const std::exception_ptr& Exception() const& {
        if (!HasException()) throw std::runtime_error("Not exception state");
...
public:
    ...
    ~Try() {
        if (state_ == State::Exception) exception_.~exception_ptr();
    }
    ...
    void Check() const {
        if (state_ == State::Exception) std::rethrow_exception(exception_);
    }
...
WrapWithTry(F&& f, Args&&... args) {
    ...
    try {
        return typename TryWrapper<Type>::Type(
