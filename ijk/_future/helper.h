...
struct CanCallWith {
    ...
    typedef decltype(
        Check<F>(nullptr)) type;  // true_type if T可以接受Args作为参数
...
struct CallableResult {
    // Test F call with arg type: void, T&&, T&, but do Not choose Try type as
    // args
    typedef typename std::conditional<
        CanCallWith<F>::value, ResultOfWrapper<F>,
        typename std::conditional<
            CanCallWith<F, T&&>::value, ResultOfWrapper<F, T&&>,
            typename std::conditional<
                CanCallWith<F, T&>::value, ResultOfWrapper<F, T&>,
                typename std::conditional<CanCallWith<F, Try<T>&&>::value,
                                          ResultOfWrapper<F, Try<T>&&>,
                                          ResultOfWrapper<F, Try<T>&>>::type>::
                type
...
struct CallableResult<F, void> {
    ...
    typedef typename std::conditional<
        CanCallWith<F>::value, ResultOfWrapper<F>,
        typename std::conditional<
            CanCallWith<F, Try<void>&&>::value, ResultOfWrapper<F, Try<void>&&>,
            ResultOfWrapper<F, Try<void>&>>::type>::type Arg;
...
template <template <typename...> class CTX, typename... Ts, typename THead,
          typename... TTail>
void CollectVariadicHelper(const std::shared_ptr<CTX<Ts...>>& ctx, THead&& head,
                           TTail&&... tail) {
