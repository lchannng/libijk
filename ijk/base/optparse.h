...
public:
    ...
    Option &action(const std::string &a) {
        ...
        if (a == "store_const" or a == "store_true" or a == "store_false" or
            a == "append_const" or a == "count" or a == "help" or
            a == "version") {
            nargs(0);
        }
        return *this;
...
private:
    std::string check_type(const std::string &opt,
        ...
        if (type() == "int" or type() == "long") {
            ...
            if (find(choices().begin(), choices().end(), val) ==
                choices().end()) {
    ...
    std::string format_option_help(const unsigned int indent = 2) const {
        ...
        if (not _short_opts.empty()) {
            ss << detail::str_join_trans(", ", _short_opts.begin(),
                                         _short_opts.end(),
        ...
        if (not _long_opts.empty()) {
            ss << detail::str_join_trans(", ", _long_opts.begin(),
                                         _long_opts.end(),
    ...
    std::string format_help(const unsigned int indent = 2) const {
        ...
        if (h.length() >= (opt_width - 1)) {
            ...
            if (help().empty()) ss << std::endl;
        }

        if (not help().empty()) {
            std::string help_str =
                (not get_default().empty())
    ...
    friend OptionParser &detail::add_option_group_helper(
        OptionParser &parser, const OptionGroup &group);
};
    ...
    Option &add_option(const std::vector<std::string> &opt) {
        ...
        for (std::vector<std::string>::const_iterator it = opt.begin();
            ...
            if (it->substr(0, 2) == "--") {
                ...
                if (dest_fallback.empty()) dest_fallback = s;
...
private:
    const Option &lookup_short_opt(const std::string &opt) const {
        std::map<std::string, Option const *>::const_iterator it =
            _optmap_s.find(opt);
        if (it == _optmap_s.end()) {
    ...
    const Option &lookup_long_opt(const std::string &opt) const {
        ...
        for (std::map<std::string, Option const *>::const_iterator it =
                 _optmap_l.begin();
             it != _optmap_l.end(); ++it) {
        ...
        if (matching.size() > 1) {
            std::string x =
                detail::str_join(", ", matching.begin(), matching.end());
    ...
    void process_opt(const Option &o, const std::string &opt,
...
static Values &parse_args_helper(OptionParser &parser,
    ...
    for (std::list<Option>::const_iterator it = parser._opts.begin();
        ...
        if (not it->get_default().empty() and
            not parser._values.is_set(it->dest())) {
            parser._values[it->dest()] = it->get_default();
        }
    }

    for (std::vector<OptionGroup const *>::iterator group_it =
             parser._groups.begin();
         group_it != parser._groups.end(); ++group_it) {
        for (std::map<std::string, std::string>::const_iterator it =
                 (*group_it)->_defaults.begin();
