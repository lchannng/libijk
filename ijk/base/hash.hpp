#pragma once
#ifndef HASH_HPP_0LRTT3VH
#define HASH_HPP_0LRTT3VH
...
inline void hash_combine(std::size_t &seed, T const &v) {
    std::hash<T> hasher;
    return ijk::hash_detail::hash_combine_impl(seed, hasher(v));
}
...
inline constexpr uint64_t chash_string(const char *str, size_t len,
                                       uint64_t seed = 0) {
    return 0 == len
               ? seed
               : chash_string(str + 1, len - 1,
                              seed ^ (static_cast<uint64_t>(*str) + 0x9e3779b9 +
                                      (seed << 6) + (seed >> 2)));
}
